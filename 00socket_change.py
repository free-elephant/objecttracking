    ...
    while True:
        ...
        if initBB is not None:
            ...
            if success:
                ...
                if point_dis<1.5: #stop
                    stop=3 #stop
                    if 230<=point<=250:
                        straight=0 #str
                        sock.send(str(straight).encode('utf-8'))
                        sock.send(str(stop).encode('utf-8'))
                    elif point<230:
                        left=1 #left
                        sock.send(str(left).encode('utf-8'))
                        sock.send(stop.encode('utf-8'))
                    elif point>250:
                        right=2 #right
                        sock.send(str(right).encode('utf-8'))
                        sock.send(str(stop).encode('utf-8'))
                elif  point_dis>3:
                    go=4 #go
                    if 230<=point<=250:
...
cam = threading.Thread(target=cam_start, args=(clientSock, True))
# sender.start()
cam.start()
while True:
    time.sleep(1)
